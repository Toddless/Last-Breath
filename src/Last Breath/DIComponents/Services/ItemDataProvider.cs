namespace LastBreath.DIComponents.Services
{
    ...
    internal class ItemDataProvider(string itemDataPath) : IItemDataProvider
    {
        ...
        public IItem CopyBaseItem(string id) =>
            TryGetItem(id)?.Copy<IItem>() ?? throw new ArgumentNullException($"Item not found: {id}");
        ...
        {
            var item = TryGetItem(recipeId);
            if (item is not ICraftingRecipe recipe || recipe == null)
                throw new ArgumentNullException($"Recipe not found: {recipeId}");
            return recipe;
        }
        ...
        public IEnumerable<ICraftingRecipe> GetCraftingRecipes() =>
        [
            .. _itemData.Values.Where(x => x is ICraftingRecipe).Cast<ICraftingRecipe>()
        ];
