namespace Utilities
{
    ...
    public static partial class Lokalizator
    {
        ...
        public static string LokalizeDescriptionFormated(string id, params object[] args)
        {
            var template = LokalizeDescription(id);

            return ContainsNamedPlaceholder(template) ? FormatNamed(template, BuildNamedDictFromArgs(args)) : FormatNumbered(template, args);

        }

        private static string LokalizeFormatNamed(string id, IDictionary<string, object> values)
        {
            var template = Lokalize(id);
